...
if 'Hash' not in df.columns:
    raise ValueError("Column 'Hash' not found in the data")

# Apply log transformation to the 'Hash' column
df['Log_Hash'] = np.log(df['Hash'])

# Split the data into training and testing sets
train_len = int(df.shape[0] * 0.8)
train_data, test_data = train_test_split(df, train_size=train_len)

y_train = train_data['Log_Hash'].values
y_test = test_data['Log_Hash'].values
...
fc, conf_int = auto.predict(n_periods=len(y_test), return_conf_int=True)
fc_original_scale = np.exp(fc)
conf_int_original_scale = np.exp(conf_int)

# Evaluate the model
mse = mean_squared_error(np.exp(y_test), fc_original_scale)
smape_val = smape(np.exp(y_test), fc_original_scale)

# Plot the results
fig, axes = plt.subplots(2, 1, figsize=(12, 12))

# Actual vs. Predicted
axes[0].plot(train_data["Month"], np.exp(y_train), color='blue', label='Training Data')
axes[0].plot(test_data["Month"], np.exp(y_test), color='red', label='Actual Price')
axes[0].plot(test_data["Month"], fc_original_scale, color='green', marker='o', label='Predicted Price')
axes[0].set_title('Hash Rate: Actual vs. Predicted')
axes[0].set_xlabel('Dates')
axes[0].set_ylabel('Hash Rate')
axes[0].legend()

# Predicted with confidence intervals
axes[1].plot(df["Month"], df["Log_Hash"], color='blue', label='Training Data')

date_range_series = pd.date_range(start=df["Month"].iloc[-1], end='2030-01-01', freq='D')

auto = pm.auto_arima(df["Log_Hash"].values, d=n_diffs, seasonal=False, stepwise=True,
                     suppress_warnings=True, max_p=6, trace=2)

fc, conf_int = auto.predict(n_periods=len(date_range_series), return_conf_int=True)
fc_original_scale = np.exp(fc)
conf_int_original_scale = np.exp(conf_int)

axes[1].plot(date_range_series, fc, color='green', label='Predicted Hash')
axes[1].fill_between(date_range_series, conf_int[:, 0], conf_int[:, 1],
                     alpha=0.95, color='orange', label="Confidence Intervals")
axes[1].set_title('Hash Rate Predictions & Confidence Intervals')
axes[1].set_xlabel('Dates')
axes[1].set_ylabel('Log Hash')
axes[1].legend()
